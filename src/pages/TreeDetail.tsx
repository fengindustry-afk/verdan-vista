  ...
  return (
    <div className="relative p-6 lg:p-8 space-y-6">
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <BentoCard>
          <h3 className="text-sm font-semibold text-foreground mb-3">Readings ({treeReadings.length})</h3>
          <div className="space-y-2 max-h-72 overflow-auto">
            {treeReadings.length === 0 && <p className="text-xs text-muted-foreground">No readings recorded.</p>}
            {treeReadings.slice().reverse().map((r) => (
              <div key={r.id} className="flex items-center justify-between text-xs border-b border-border/40 pb-2">
